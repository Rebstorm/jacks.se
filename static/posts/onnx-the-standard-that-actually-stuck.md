...
- **Inputs/outputs** are the boundary of the graph — what you feed in, what comes out
...
Opset 17 means "I'm using the ONNX standard operators at version 17." When an op changes behavior — say `Resize` added new coordinate transformation modes in opset 11, and `BatchNormalization` changed its training vs inference semantics in opset 14 — a new opset version captures that.
